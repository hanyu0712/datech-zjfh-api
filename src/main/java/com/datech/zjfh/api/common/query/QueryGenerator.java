...
import com.datech.zjfh.api.common.consts.GlobalConstant;
import com.datech.zjfh.api.common.model.PermDataRuleModel;
import com.datech.zjfh.api.util.DBUtil;
import com.datech.zjfh.api.util.DateUtil;
import com.datech.zjfh.api.util.SqlInjectionUtil;
...
public class QueryGenerator {
    ...
    private static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap) {
		...
		*/
        ...
        Map<String, PermDataRuleModel> ruleMap = getRuleMap();

        //权限规则自定义SQL表达式
        for (String c : ruleMap.keySet()) {
            if(oConvertUtil.isNotEmpty(c) && c.startsWith(SQL_RULES_COLUMN)){
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                ...
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    ...
                    final String field = oConvertUtil.camel2Underline(column);
                    if(vals.length>1) {
                        queryWrapper.and(j -> {
    ...
    private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
        ...
        if (oConvertUtil.isNotEmpty(column) && oConvertUtil.isNotEmpty(order)) {
            //字典字段，去掉字典翻译文本后缀
            if(column.endsWith(GlobalConstant.DICT_TEXT_SUFFIX)) {
                column = column.substring(0, column.lastIndexOf(GlobalConstant.DICT_TEXT_SUFFIX));
            }
            //SQL注入check
            SqlInjectionUtil.filterContent(column);

            if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
                queryWrapper.orderByAsc(oConvertUtil.camel2Underline(column));
            } else {
                queryWrapper.orderByDesc(oConvertUtil.camel2Underline(column));
            }
        }
    }
    ...
    private static void doSuperQuery(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap, Map<String,String> fieldColumnMap) {
        if(parameterMap!=null&& parameterMap.containsKey(SUPER_QUERY_PARAMS)){
            ...
            try {
                ...
                List<com.datech.zjfh.api.common.query.QueryCondition> conditions = JSON.parseArray(superQueryParams, com.datech.zjfh.api.common.query.QueryCondition.class);
                ...
                queryWrapper.and(andWrapper -> {
                    for (int i = 0; i < conditions.size(); i++) {
                        com.datech.zjfh.api.common.query.QueryCondition rule = conditions.get(i);
                        if (oConvertUtil.isNotEmpty(rule.getField())
                                && oConvertUtil.isNotEmpty(rule.getRule())
                                && oConvertUtil.isNotEmpty(rule.getVal())) {

                            log.debug("SuperQuery ==> " + rule.toString());
                            addEasyQuery(andWrapper, fieldColumnMap.get(rule.getField()),
                                    QueryRuleEnum.getByValue(rule.getRule()), rule.getVal());
    ...
    private static void addQueryByRule(QueryWrapper<?> queryWrapper,String name,String type,String value,QueryRuleEnum rule) throws ParseException {
        if(oConvertUtil.isNotEmpty(value)) {
            Object temp;
            // 针对数字类型字段，多值查询
    ...
    private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
        if (value == null || rule == null || oConvertUtil.isEmpty(value)) {
            return;
        }
        name = oConvertUtil.camel2Underline(name);
     ...
     * @return
     */
    private static Map<String, PermDataRuleModel> getRuleMap() {
        return getRuleMap(null);
    }

    private static Map<String, PermDataRuleModel> getRuleMap(List<PermDataRuleModel> list) {
        Map<String, PermDataRuleModel> ruleMap = new HashMap<>();
        /*if(list == null){
            list = DataPermUtil.getPermDataRulesFromRpcContext();
        }*/
        if(list != null && list.size() > 0) {
            if(list.get(0) == null){
                return ruleMap;
            }
            for (PermDataRuleModel rule : list) {
    ...
    private static void addRuleToQueryWrapper(PermDataRuleModel dataRule, String name, Class propertyType,
...
//        String value = DataPermUtil.getUserSystemData(ruleValue);
//        return value != null ? value : ruleValue;
    ...
    public static String trimSingleQuote(String ruleValue) {
        if (oConvertUtil.isEmpty(ruleValue)) {
            return "";
        }
    ...
    public static String getSingleQueryConditionSql(String field, String alias, Object value, boolean isString) {
        ...
        field = alias + oConvertUtil.camel2Underline(field);
    ...
    public static String getSingleSqlByRule(QueryRuleEnum rule, String field, Object value, boolean isString) {
        ...
        switch (rule) {
            ...
            case LIKE:
                ...
                res = field + " like "+getLikeConditionValue(str);
                break;
            case LEFT_LIKE:
                if(!str.startsWith("*")) {
                    str = "*" + str;
    ...
    private static String getFieldConditionValue(Object value, boolean isString) {
        ...
        if(isString) {
            if(DBConstant.DB_TYPE_SQLSERVER.equals(getDbType())){
                return " N'"+str+"' ";
            }else{
                return " '"+str+"' ";
            }
            ...
            if(DBConstant.DB_TYPE_SQLSERVER.equals(getDbType()) && str.endsWith("'") && str.startsWith("'")){
                return " N"+str;
            }
    ...
    private static String getLikeConditionValue(String str) {
    ...
    public static String installAuthJdbc(Class<?> clazz) {
        return installAuthJdbc(clazz, "");
    }

    public static String installAuthJdbc(Class<?> clazz, String alias) {
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            if(ruleMap.containsKey(name)) {
                ...
                PermDataRuleModel dataRule = ruleMap.get(name);
                QueryRuleEnum rule = QueryRuleEnum.getByValue(dataRule.getRuleConditions());
                ...
                String field = oConvertUtil.camel2Underline(column);
                if(oConvertUtil.isNotEmpty(alias)) {
                    field = alias.trim() + "." + oConvertUtil.camel2Underline(column);
                }
                String filedSql = getSingleSqlByRule(rule, field, value, isString);
                sb.append(sql_and + filedSql);
            }
        }
        log.info("数据权限SQL: {}", sb.toString());
        return sb.toString();
    }
    ...
    public static String getAllConfigAuth() {
        ...
        String sql_and = " and ";
        for (String c : ruleMap.keySet()) {
            PermDataRuleModel dataRule = ruleMap.get(c);
